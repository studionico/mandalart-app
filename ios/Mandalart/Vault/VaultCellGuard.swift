...
///
/// `shared/vault-fixtures` の `cellGuard` kind が desktop と本ファイルを同一契約で検証する。
/// Foundation のみ依存・SwiftData/Supabase 非依存 (Vault ピュア層の規約)。
///
...
/// 空判定 / 周辺判定に必要な最小フィールド。`VaultCell` と production の `SlotCell` が conform する。
...
extension VaultCell: CellGuardCell {}
