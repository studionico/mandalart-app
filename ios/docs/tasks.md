...
- [ ] **Stage I/O**: UIDocumentPicker + security-scoped bookmark (iCloud Drive、Obsidian 互換) で
  フォルダ選択・永続化、FileManager で scan/write/remove、画像 attachments (imageVault 相当)。
- [ ] **Stage DB**: applyToDb 相当を SwiftData modelContext upsert (id で insert/update 分岐、
  soft-delete 復活、folder_name→folder_id 解決) に翻訳。dbRows 相当は FetchDescriptor。
- [ ] **Stage 反転 + 同期 gate**: 起動時 rebuild + Settings トグル、vaultMode 中は SyncEngine /
  fullSync を `!vaultMode` で gate (desktop P4 と対称、落とし穴 #24 復帰条件にも vaultMode を含める)。
- [ ] **Stage auto-flush**: flushScheduler 相当 (DispatchSourceTimer 等) + DB 編集 → debounce → flush。
