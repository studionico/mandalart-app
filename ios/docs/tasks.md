...
- [x] ~~**定期 auto-push (15 秒間隔)**~~ / ~~**realtime 購読** ([`RealtimeService`](../Mandalart/Services/RealtimeService.swift))~~ — **⚠️ 2026-05-04 緊急停止中**。Supabase Realtime Messages 過剰使用警告 (ルート [`CLAUDE.md`](../../CLAUDE.md) 冒頭 / desktop 落とし穴 #24) により [`MandalartApp`](../Mandalart/App/MandalartApp.swift) でコメントアウト済。現状はサインイン直後 fullSync + 手動「今すぐ同期」のみ。復帰条件はルート CLAUDE.md のチェックリスト参照 (15 秒 polling は廃止し mutation 駆動 dirty flag + 60 秒 debounce に置換予定)
...
- [x] **JSON / Markdown / IndentText の Export** ([`TransferService.swift`](../Mandalart/Services/TransferService.swift)):
  ...
  - **export はエディターに集約** (desktop と同様 dashboard には export を置かない。2026-06-07 に dashboard export を撤去):
    cell 長押し context menu で JSON/MD/Indent (= **そのセル以下の subtree**) + PNG/PDF (= 現在グリッド画像)。`exportCellAsSnapshot` で
    動作分岐: 中心セルなら自グリッド全体 / 周辺で drilled あれば drilled grid を出力 (cell が center として merge) / 周辺で drilled なしなら
    synthetic snapshot (cell content のみ)。iPad は 9×9 表示中に専用ボタンで全体ボード画像。Export はロック中も許可 (= 読み取り専用)
...
- [x] **PNG / PDF Export** (2026-06-07) — [`MandalartImageRenderer`](../Mandalart/Services/MandalartImageRenderer.swift) で
  `ImageRenderer`→PNG / `UIGraphicsPDFRenderer`→単一ページ PDF。エントリは **エディターのセル長押しメニュー (3×3=現在グリッド)** と
  **iPad の 9×9 ボタン (全体ボード)** に集約 (dashboard には置かない=desktop 準拠)。**画面と同じ gridSize・readOnly:false・ライト固定**で
  描画し見た目を画面と一致させる (`MandalartExportDocument` に `writableContentTypes` で png/pdf を定義し拡張子崩れ防止)。罠は
  [`pitfalls.md`](pitfalls.md) #16。実機確認済。95 tests green、build SUCCEEDED
...
## Phase 12: vault フォルダモード (Markdown ファイルを正) — ❌ 撤去済 (ローカル保存は全廃)

> **2026-06-08 撤去 (第 1 段)**: 双方向 Markdown vault (`.md` を canonical source とし外部編集を DB に取り込む、
> 本文ラウンドトリップ / reconcile / write ledger・echo-skip / vaultMode によるクラウド同期停止 / 起動時の
> vault→DB 再構築 / scenePhase の書き出し⇄取り込み往復) は **完全に廃止**された。`Mandalart/Vault/` ディレクトリ・
> `Services/Vault*` (VaultRowsBridge / VaultDbApply / VaultDbReconcile / VaultAutoFlush / VaultExitSync)・
> `MandalartTests/Vault*Tests.swift` + `GoldenFixtureTests.swift`・`shared/vault-fixtures/` はすべて削除済。
> `MandalartApp.swift` から bootstrap rebuild / scenePhase `.active` import / `fullSync` の vaultMode early-return を撤去。
> 一旦より単純な一方向ローカル JSON ミラー (`Services/Mirror*` + `SecurityScopedBookmark`) に置換した。
>
> **2026-06-08 撤去 (第 2 段)**: その JSON ミラーも「複雑さに見合わない」と判断し撤去。**ローカルファイル保存は全廃**し、
> クラウド同期 (subscribe) + 必要時の **手動 export** ([`TransferService.swift`](../Mandalart/Services/TransferService.swift)、
> JSON / Markdown / PNG / PDF) に一本化した。`Services/Mirror*` ・`SecurityScopedBookmark` ・`MirrorFilenameTests` を削除し、
> `MandalartApp` から MirrorAutoFlush / scenePhase flush を、`SettingsView` からミラー Section を撤去。セル空判定 / 中心セル保護の
> 純粋判定は `Mandalart/Utils/CellGuard.swift` に存続 (型名 `CellGuard` / `CellGuardCell` は不変)。`LogicTests` スキームは
> CellGuard のみコンパイルする (MirrorFilename を target source から除外)。
> 以下の旧チェックリストは歴史記録として残す (すべて撤去済)。

- [x] ~~**Stage 0/1: ピュア層移植 + ユニットテスト** (2026-06-06)~~ — 撤去済。desktop `src/lib/vault/` の純変換層を
...
- [x] **本文ラウンドトリップ Stage ④ clobber 安全化 (echo-skip)** (2026-06-06) — flush (DB→vault) が前面中に届いた外部編集を
  上書きする穴を塞ぐ。新規 [`VaultWriteLedger`](../Mandalart/Vault/VaultWriteLedger.swift) (per-file「自分の最後の書込み hash」台帳、
  process メモリのみ) + ピュア [`diffFilesGuarded`](../Mandalart/Vault/VaultReconcile.swift) で、disk が台帳と違えば外部編集とみなし
  上書き/削除を skip (次回 reconcile で取り込む)。reconcile が現 disk hash を seed して Stage ③ の frontmatter 整合書込みを維持。
  台帳引数は optional (既定 nil=従来 legacy 挙動) で本番外経路・既存テスト不変。`flushDbToVault` は台帳 (MainActor) 参照のため
  `@MainActor` 化。**93 tests green** (Stage ④ +10/−1)、Debug build SUCCEEDED。
- [x] **vault OFF 遷移時の updated_at 整備** (2026-06-06) — vault モード中は本文編集の updatedAt が stale なため OFF 後の
  クラウド再同期で ① 未 push (needsPush=false) ② pull の LWW で clobber される穴があった。新規
  [`VaultExitSync.markLocalRowsDirty`](../Mandalart/Services/VaultExitSync.swift) を [`SettingsView.setVaultMode(false)`](../Mandalart/Views/SettingsView.swift)
  に配線し、OFF 時に全行 `updatedAt=now` へ bump (syncedAt 不変)。pull-LWW が false になり clobber されず、needsPush=true で
  push され、cloud の BEFORE UPDATE トリガで cloud 最新化 → local/vault 全面勝ち。実 push は次回起動 fullSync / 手動ボタンに委譲
  (自動同期しない)。tombstone も bump。実機確認 OK。**95 tests green**、Debug build SUCCEEDED。
  残: フォルダ watcher (起動中ライブ取り込み、現状は背面=flush・復帰=reconcile で代替) / 大規模 vault 差分 rebuild 最適化。
- [x] **本文 parser を desktop と parity 化 (ブロック parse + clean 削除)** (2026-06-07) — vault フォルダを desktop/iOS で共有運用したとき、
  iOS の `parseGridBody` が **行単位**だったため ① 複数行テキストセルの本文編集 (render は両 OS とも内部改行を保持し `^pN` が継続行に来る)
  を取りこぼし、② 本文での `## 見出し` 削除が効かない (clean 非対称) 乖離があった。[`VaultBody`](../Mandalart/Vault/VaultBody.swift) を
  desktop [`vaultBody.ts`](../../desktop/src/lib/vault/vaultBody.ts) の逐語移植にし、`parseGridBody` を**ブロック単位** (`parseHeadingBlock`)
  へ、`BodyParse.clean` + `mergeBody` の「クリーンな本文で消えた position は削除 (中心セル除外)」へ拡張。Stage ①② の「欠落 position 維持」は
  clean=false (グリッチ) 時のみのフォールバックに変更。[`VaultDbApply`](../Mandalart/Services/VaultDbApply.swift) の cell 削除に
  `referencedCellIds` (grid の centerCellId/parentCellId) 孤児ガードを移植。共有 golden fixture (`shared/vault-fixtures/body-multiline-heading`
  / `body-clean-delete` / `body-unclean-keep`) を追加し両言語 loader で `clean` を検証 (TS↔Swift parity を CI で恒久ロック)。
