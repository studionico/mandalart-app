...
- [ ] **realtime 購読**: `client.realtime.channel(...).on(.postgresChanges, ...)` で他端末の変更を即時反映 (現状は scene phase 復帰時 pull で代替)
...
- [ ] **マンダラート作成時に `folder_id` を Inbox folder の id にセットする** (現状 `MandalartFactory` は nil で作成、desktop 側 [`adoptOrphanMandalartsToInbox()`](../../desktop/src/lib/api/folders.ts) の保険で動作中。iOS 側でも folders を pull → Inbox を特定 → 新規 mandalart の `folderId` をセット、が必要)
