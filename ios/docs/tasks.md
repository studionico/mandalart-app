...
## Phase 12: vault フォルダモード (Markdown ファイルを正) — ❌ 撤去済 (ローカル JSON ミラーへ置換)

> **2026-06-08 撤去**: 双方向 Markdown vault (`.md` を canonical source とし外部編集を DB に取り込む、
...
>
> **置換**: より単純な **一方向ローカル JSON ミラー** (DB → ファイルのみ、watcher / 取り込みなし、クラウド同期を
> 止めない) に置き換えた。`Services/Mirror*` (MirrorConfig / MirrorFilename / MirrorSync / MirrorAutoFlush) +
> `SecurityScopedBookmark`、設定は UserDefaults キー `mirror.enabled` / `mirror.bookmark` / `mirror.path`、UI は
> `SettingsView` (出荷、DEBUG 限定ではない)。セル空判定 / 中心セル保護の純粋判定は `Mandalart/Utils/CellGuard.swift`
> へ移設 (型名 `CellGuard` / `CellGuardCell` は不変)。テストスキームは `VaultTests` → `LogicTests` に改名し、ピュアな
> CellGuard + MirrorFilename のみコンパイルする。仕様は desktop [`requirements.md`](../../desktop/docs/requirements.md#ローカル-json-ミラー) /
> [`api-spec.md`](../../desktop/docs/api-spec.md#libmirror--ローカル-json-ミラー) を参照。
