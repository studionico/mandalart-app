...
- **#5 Realtime DELETE は子行を連鎖しない** — realtime は 2026-05-04 緊急停止中 (ルート [`CLAUDE.md`](../../CLAUDE.md))。復帰時は明示カスケード必須
- **#10 中心セル行の有無は grid 種別で 3 パターン** (root / X=C primary drilled / レガシー共有並列) — エディタ画面 (Phase 5) を組むとき必読
- **#12 zombie cleanup** — push 失敗 thrash 対策。**iOS 実装済**: [`SyncEngine.sanitizeZombies`](../Mandalart/Services/SyncEngine.swift) が pullAll/pushPending 冒頭で参照整合性をサニタイズ
- **#17 PGRST204 thrash** — Supabase 手動 ALTER 漏れ防止。スキーマ追加時は両プラットフォーム同時更新
- **#6 完全削除は cloud + local 両方** — **iOS 実装済**: [`MandalartFactory.permanentDelete`](../Mandalart/Services/MandalartFactory.swift) が local 削除後に cloud cascade delete、失敗時は [`CloudDeleteTombstone`](../Mandalart/Services/CloudDeleteTombstone.swift) でリトライ

> 上記 `#N` は **desktop/CLAUDE.md の落とし穴番号**。下の「iOS 固有」セクションの `#N` は**本ファイル内ローカル番号**で別系統 (同じ番号でも内容は無関係)。
