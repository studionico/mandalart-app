...
- **breadcrumb = 現在地マップアイコン列**: 各階層をテキストではなく **3×3 ミニマップアイコン** で表す。アイコンはその階層で「展開したセルの position」を 1 マス塗る (ルート = 中心 / 以降 = 親で展開した周辺セル位置)。横並びで「ルート中心 → 展開した周辺 → …」の経路マップになる。アイコンは小さく均一なので折りたたみはせず全項目を chevron 区切りで横スクロール表示、元セルテキストは `.accessibilityLabel` に退避 ([`Breadcrumb.swift`](../Mandalart/Views/Components/Breadcrumb.swift))。**iPhone (= compact horizontal size class) では breadcrumb を表示せず、右ペイン (memo/stock) 上部に `[arrow.up.to.line]` (root 戻る、depth>1 のみ可視 / 36pt 枠は常時保持) → `[showCheckbox toggle]` → `[文字サイズ capsule]` の 3 controls を HStack(spacing:8) で左寄せ並置** (memoW タイト機種向けに `ScrollView(.horizontal)` でガード、中間階層へのジャンプは割愛)。iPad (regular) は従来通り breadcrumb 表示 + 各 controls は右上 floating
...
- **兄弟セルナビ「隣のテーマ」** (機能仕様は desktop canonical [`requirements.md`](../../desktop/docs/requirements.md) 「兄弟セルナビゲーション」節を参照): サブグリッド編集中 (breadcrumb 2 段以上) かつ 3×3 のとき、親グリッドの隣の周辺セルのサブグリッドへ横移動する。**配置**: グリッド両脇の既存ナビスロットに集約 — 並列ナビの単一 chevron (`chevron.left`/`chevron.right`) の **上に二重 chevron (`chevron.left.2`/`chevron.right.2`) を縦積み** (`parallelNavButton` を再利用)。iPad / iPhone 共通配置で、横スクロール行に載せないため見切れない。スロット幅は 36pt 据え置きで `gridSize` に影響しない。巡回順は `GridConstants.peripheralPositionsByTab` (時計回り `[7,6,3,0,1,2,5,8]`)。空セルは着地時に lazy 作成、ロック中は既存子グリッドを持つセルのみ巡回。実装: [`EditorView.handleSiblingNav`](../Mandalart/Views/EditorView.swift)
- **セル左上 done チェックボックス** (`Mandalart.showCheckbox == true` 時のみ): 22pt 視覚 + 30pt hit area、非空 cell 全てに表示 (中心 / 周辺 / 子あり / 画像セル どれでも)。tap で `Cell.done` をトグル + サブツリーへ down 伝播 + 親方向へ up 伝播 ([`CellCheckboxService`](../Mandalart/Services/CellCheckboxService.swift))。9×9 inner / 編集中 / 空セルでは非表示。**ロック中は visible (= 閲覧情報) だが tap で no-op** ([`EditorView.handleToggleDone`](../Mandalart/Views/EditorView.swift) で `m.locked` ガード)。desktop [`Cell.tsx:336-412`](../../desktop/src/components/editor/Cell.tsx) と挙動一致
  - **新規入力セルは必ず未完了**: 空→非空遷移 (新たに入力) では `Cell.done` を false に固定し ([`EditorView.commitEditing`](../Mandalart/Views/EditorView.swift))、`CellCheckboxService.propagateUndoneUpward` で親 done を解除する。これにより cut / 内容クリア後に残った stale done が再入力時に表示される問題を防ぐ。既存非空セルの text 編集 (typo 修正等) では done を保持。desktop [`updateCell`](../../desktop/src/lib/api/cells.ts) と挙動一致
- **セル入れ替え / ストックペースト の選択モード UI (共通方針)**: 上部 banner は出さず、視覚 cue は **source 表示元** に集約する (= 縦スペース節約):
  - **セル入れ替え (tap-select)**: desktop の D&D swap を iOS で代替。CellView の長押し context menu に「入れ替え」 (周辺 + 非空 + 非ロック時のみ表示) → source 枠を accent color highlight → 周辺 cell tap で確定 (空 slot も target 可) / source 再 tap で cancel。中心セル絡みは context menu 非表示 + target 時 alert 拒否 (判定は display slot position で実施、child grid の merged center 対策)。実装: [`CellSwapService.swap`](../Mandalart/Services/CellSwapService.swift) が text / imagePath / color / done と grids の parentCellId / centerCellId を双方向 swap (`done` も内容に付随して swap、自グリッド除外で root 自己参照保持。同一 grid swap なので done 集合不変 → 中心 done 再計算不要)
...
- `@Environment(\.colorScheme)` で自動対応 ([`CellView`](../Mandalart/Views/Components/CellView.swift) 等)
- グローバル手動 override を [`ThemePreference`](../Mandalart/Utils/ThemePreference.swift) で実装済 (UserDefaults キー `app.theme` / 3 値 `light` / `system` / `dark`、初期値 `system`)。[`MandalartApp`](../Mandalart/App/MandalartApp.swift) で `.preferredColorScheme(_:)` を root に適用 (`system` 時は `nil` で OS 追従)
- 切替 UI ([`ThemeToggle`](../Mandalart/Views/Components/ThemeToggle.swift)) の配置: Editor 右上 floating (iPad regular) / 右ペイン上部 HStack (iPhone compact) / Dashboard toolbar primaryAction / [`SettingsView`](../Mandalart/Views/SettingsView.swift) 「外観」Section
- 背景色は [`NeutralPalette`](../Mandalart/Utils/NeutralPalette.swift) で Tailwind neutral 系列を直 RGB で保持 (`systemBackground` 等の semantic color は意図的に回避し desktop と RGB 完全一致を優先)。プリセット色は [`PresetColors.backgroundColor(for:)`](../Mandalart/Utils/PresetColors.swift) が `ColorScheme` を受けて light/dark 値を分岐
- desktop の localStorage key は `mandalart.theme`、iOS は `app.theme` と意図的に異なる (cross-device 同期しない明示)
