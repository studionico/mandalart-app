...
import { deleteMandalart, duplicateMandalart, updateMandalartTitle } from '@/lib/api/mandalarts'
...
export default function MandalartCard({ mandalart, previewCells, onDeleted, onUpdated }: Props) {
  ...
  function handleOpen() {
    router.push(`/mandalart/${mandalart.id}`)
  }
  ...
  async function handleDuplicate() {
    await duplicateMandalart(mandalart.id)
    router.refresh()
  }

  async function handleRename() {
    const updated = await updateMandalartTitle(mandalart.id, renameValue)
    onUpdated(updated)
    setRenameOpen(false)
  }
