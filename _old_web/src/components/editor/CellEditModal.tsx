...
import { PRESET_COLORS, getColorClasses } from '@/constants/colors'
import { TAB_ORDER, nextTabPosition } from '@/constants/tabOrder'
...
export default function CellEditModal({
  ...
  const handleSave = useCallback(async () => {
    ...
    onClose()
  }, [cell, text, imagePath, color, onSave, onClose])
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const path = await uploadCellImage({ file, userId, mandalartId, cellId: cell.id })
  ...
  function handleKeyDown(e: React.KeyboardEvent) {
    ...
    if (e.key === 'Tab') {
      ...
      // 中心セルが空なら移動しない
      const centerCell = allCells.find((c) => c.position === 4)
      if (centerCell && isCellEmpty(centerCell) && cell.position === 4) return
  ...
  if (isMobile) {
    return (
      <BottomSheet open={!!cell} onClose={onClose} title="セルを編集">
        {content}
      </BottomSheet>
    )
  }
  ...
  return (
    <Modal open={!!cell} onClose={onClose} title="セルを編集">
      {content}
    </Modal>
  )
}
