...
export async function getRootGrids(mandalartId: string): Promise<Grid[]> {
  return query<Grid>(
    'SELECT * FROM grids WHERE mandalart_id = ? AND parent_cell_id IS NULL ORDER BY sort_order',
    [mandalartId]
  )
}
...
export async function getChildGrids(parentCellId: string): Promise<Grid[]> {
  return query<Grid>(
    'SELECT * FROM grids WHERE parent_cell_id = ? ORDER BY sort_order',
    [parentCellId]
  )
}

export async function getGrid(id: string): Promise<Grid & { cells: Cell[] }> {
  const grids = await query<Grid>('SELECT * FROM grids WHERE id = ?', [id])
  const grid = grids[0]
  if (!grid) throw new Error(`Grid not found: ${id}`)
  const cells = await query<Cell>('SELECT * FROM cells WHERE grid_id = ? ORDER BY position', [id])
  return { ...grid, cells }
}
...
export async function createGrid(params: {
  ...
  const cells = await query<Cell>('SELECT * FROM cells WHERE grid_id = ? ORDER BY position', [gridId])
  const grid: Grid = {
    id: gridId,
