...
async function applyMandalartChange(payload: { eventType: string; new: Record<string, unknown>; old: Record<string, unknown> }): Promise<boolean> {
  ...
  const local = await query<{ title: string; root_cell_id: string; deleted_at: string | null; updated_at: string }>(
    'SELECT title, root_cell_id, deleted_at, updated_at FROM mandalarts WHERE id = ?',
    [m.id],
  )
  if (local.length === 0) {
    await execute(
