...
export async function pullAll(): Promise<{ mandalarts: number; grids: number; cells: number }> {
  ...
  const [m, g, c] = await Promise.all([
    supabase.from('mandalarts').select('id, title, root_cell_id, show_checkbox, last_grid_id, created_at, updated_at, deleted_at'),
  ...
  for (const cm of cloudMandalarts) {
    ...
    if (local.length === 0) {
      await tryInsert('mandalarts',
        ...
        [cm.id, cm.title, cm.root_cell_id, cm.show_checkbox ? 1 : 0, cm.last_grid_id ?? null, cm.created_at, cm.updated_at, cm.deleted_at, cm.updated_at],
      )
      mCount++
      ...
      await tryUpdate('mandalarts',
        ...
        [cm.title, cm.root_cell_id, cm.show_checkbox ? 1 : 0, cm.last_grid_id ?? null, cm.updated_at, cm.deleted_at, cm.updated_at, cm.id],
      )
      mCount++
    }
  }
