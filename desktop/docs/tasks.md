...
## フェーズ V (vault): Markdown ファイルを正にする外部編集モード ❌ 撤去済 (ローカル JSON ミラーへ置換)

> **2026-06-08 撤去**: Obsidian 風の双方向 Markdown vault (`.md` を canonical source とし外部編集を DB に取り込む、
...
> `vaultStore` / `bootstrapStore`・関連テスト / fixture はすべて削除済。
>
> **置換**: より単純な **一方向ローカル JSON ミラー** (DB → ファイルのみ、file watcher / 取り込みなし、クラウド同期を
> 止めない) に置き換えた。詳細は [`requirements.md`](requirements.md#ローカル-json-ミラー) /
> [`api-spec.md`](api-spec.md#libmirror--ローカル-json-ミラー) / [`folder-structure.md`](folder-structure.md) を参照。
