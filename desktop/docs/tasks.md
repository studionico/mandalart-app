...
## フェーズ V (vault): Markdown ファイルを正にする外部編集モード ✅

Obsidian 風の Markdown vault を canonical source にし、外部エディタでの `.md` 編集を DB に取り込む
モード。安全段階ロールアウト (Stage 0/1 → watch/reconcile → 実書込み → 双方向 flush → Productize)
で実装。**vaultMode は引き続き開発者向け扱い (本番昇格は別タスク)**。実装詳細はここに複製せず
[`CLAUDE.md`](../CLAUDE.md) 落とし穴 #24 末尾 / [`api-spec.md`](api-spec.md) / [`folder-structure.md`](folder-structure.md) を参照。
iOS 移植は [`ios/docs/tasks.md`](../../ios/docs/tasks.md) Phase 12。

- [x] **Stage 0/1: ピュア層 + テスト** (2026-06-02 `4d55174`) — `src/lib/vault/` の純変換層 (format / model / reconcile)
- [x] **Stage 2: fs watch + read-only reconcile** (2026-06-02 `6fc3791`) — DB 無改変で外部編集を検知
- [x] **Stage 3a–3c: DB↔vault 双方向 flush** (2026-06-02〜03 `f514b4e` / `739ebbe` / `190d965` / `ea316c2`) — file→DB 再構築 + DB→file 差分 flush で双方向ループ完成 (dev-gated)
- [x] **Phase 2 Productize P1–P4** (2026-06-03〜04 `d3ad594` / `fda3180` / `de53595` / `ab21bf8`) — Vault Settings モーダル + 手動 flush / auto-flush (debounce) / vaultMode トグル + 起動時 canonical 反転 / vaultMode 中は Supabase 同期を完全オフ
- [x] **Obsidian 双方向 wiki-link + 画像 attachments 化** (2026-06-06 `c6cb182` / `e4809b4`) — 親子 wiki-link 出力 + セル画像を vault `attachments/` に (cloud 非依存)
- [x] **本文ラウンドトリップ + ライブ watcher + clobber 安全化** (2026-06-07 `47380d8` / `f1527a6` / `29af041`) — frontmatter ではなく `.md` 本文 (人間可読) を編集→DB 反映。`useVaultWatcher` が変更フォルダだけ取り込み、`writeLedger` で flush↔watcher の echo-skip、`^pN` anchor 破損時は keep-all で誤削除回避、中心セル / 参照セルは孤児ガードで保護。改行入りセルテキストは wiki-link を単一行化して保護
- [x] **TS/Swift 共有 golden fixture** (2026-06-07 `f132907`) — クロスプラットフォーム乖離の自動検出

残: vaultMode 本番昇格 / 大規模 vault の差分 rebuild 最適化。
