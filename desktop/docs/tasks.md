...
## フェーズ V (vault): Markdown ファイルを正にする外部編集モード ❌ 撤去済 (ローカル保存は全廃)

> **2026-06-08 撤去 (第 1 段)**: Obsidian 風の双方向 Markdown vault (`.md` を canonical source とし外部編集を DB に取り込む、
> file watcher / 本文ラウンドトリップ / write ledger・echo-skip / vaultMode によるクラウド同期停止 / 起動時の
> vault→DB 再構築) は **完全に廃止**された。`src/lib/vault/` ディレクトリ・`useVaultAutoFlush` / `useVaultWatcher`・
> `vaultStore` / `bootstrapStore`・関連テスト / fixture はすべて削除済。一旦より単純な一方向ローカル JSON ミラー
> (DB → ファイルのみ) に置換した。
>
> **2026-06-08 撤去 (第 2 段)**: その JSON ミラーも「複雑さに見合わない」と判断し撤去。**ローカルファイル保存は全廃**し、
> クラウド同期 (subscribe) + 必要時の **手動 export** ([`transfer.ts`](../src/lib/api/transfer.ts) / [`export.ts`](../src/lib/utils/export.ts)、
> JSON / Markdown / PNG / PDF) に一本化した。`src/lib/mirror/` ・`useMirrorAutoFlush` ・`SettingsDialog` ・
> ミラー専用 Tauri 権限 (fs text-file / read-dir / dialog) は削除済。
> 以下の旧チェックリストは歴史記録として残す (すべて撤去済)。

- [x] ~~**Stage 0/1: ピュア層 + テスト** (2026-06-02 `4d55174`)~~ — 撤去済
- [x] ~~**Stage 2: fs watch + read-only reconcile** (2026-06-02 `6fc3791`)~~ — 撤去済
- [x] ~~**Stage 3a–3c: DB↔vault 双方向 flush** (2026-06-02〜03)~~ — 撤去済
- [x] ~~**Phase 2 Productize P1–P4** (2026-06-03〜04)~~ — 撤去済
- [x] ~~**Obsidian 双方向 wiki-link + 画像 attachments 化** (2026-06-06)~~ — 撤去済
- [x] ~~**本文ラウンドトリップ + ライブ watcher + clobber 安全化** (2026-06-07)~~ — 撤去済
- [x] ~~**TS/Swift 共有 golden fixture** (2026-06-07)~~ — 撤去済
