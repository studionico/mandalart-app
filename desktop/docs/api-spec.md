...
// クリップボードからのペースト。現在 mode='copy' (live 参照複製) でのみ使用する。
...
// ※ カットは即削除 + snapshot 退避方式 (clipboardStore.setCut + pasteSnapshot) に移行したため、
//    カットのペーストは pasteCell ではなく stock の pasteSnapshot / pasteSnapshotReplacing を使う。
...
isSelfCenterWithPeripheralContent(cellId: string): Promise<boolean>
...
// 指定セル (とそのサブツリー) の detached スナップショットを構築する。
// ストック追加 / クリップボードのカット (即削除前の退避) で共用する。
// - 周辺セル: parent_cell_id = cellId の全 grids を子として再帰スナップショット
// - 中央セル: center_cell_id = cellId のグリッド (root parallels 含む) を再帰スナップショット
buildCellSnapshot(cellId: string): Promise<CellSnapshot>
...
// detached な CellSnapshot をターゲットセルに挿入する本体 (content 上書き + 子グリッド再帰挿入)。
// 周辺セルなのに中心セルが空ならエラー。pasteFromStock とクリップボードのカット paste で共用する。
pasteSnapshot(snapshot: CellSnapshot, targetCellId: string): Promise<void>

// 入力ありセルへの snapshot ペースト (置換版)。既存サブグリッド破棄後に pasteSnapshot。
// ストックの置換 / クリップボードのカット paste (上書き確認後) で共用する。
pasteSnapshotReplacing(snapshot: CellSnapshot, targetCellId: string): Promise<void>
```
...
```typescript
// copy: 元セルを live 参照 (sourceCellId)。paste 時に DB から読み直して copyCellSubtree。
// cut : 元セルを即削除するため、削除前に取った detached snapshot を保持する。
//       paste 時は pasteSnapshot で復元、貼らなければ消えたまま (= シュレッダー)。
mode: 'cut' | 'copy' | null
sourceCellId: string | null   // copy 用
snapshot: CellSnapshot | null // cut 用
