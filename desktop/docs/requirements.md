...
- **追加ボタン `+`**: 3×3 / 9×9 グリッドの右側。末尾 **かつ周辺セルに 1 つ以上入力がある場合のみ表示**。クリックで新しい並列グリッドを作成。**中心セルは元グリッドと同じ `cells` 行を共有する** (X=C 統一モデル) → 並列グリッド間で中心の編集が DB レベルで自動同期される。中心セル単独で判定しないのは、drilled grid の中心は親 X と共有で常に非空になるため
