...
- **永続化スコープ: per-mandalart × per-device**。キー `mandalart.fontLevel.<mandalartId>` (`localStorage`) に保存。エディタに入る (= `setMandalartId(id)` 呼び出し) たびに per-mandalart key を再 load し store に反映 ([`editorStore.ts`](../src/store/editorStore.ts))
- 旧グローバルキー `mandalart.fontLevel` ([`STORAGE_KEYS.fontLevel`](../src/constants/storage.ts)) は per-mandalart キー未設定時の **全マンダラートのデフォルト** として fallback に残す。一度ユーザーが個別調整したマンダラートはその値が優先 (= 未調整マンダラートは旧グローバル値で開く)
- iOS も同じスコープ (per-mandalart × per-device)、同じキー prefix `mandalart.fontLevel.<id>` を使用 (storage backend は UserDefaults)
